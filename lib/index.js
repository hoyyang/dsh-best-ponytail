/**
 * dsh-ponytail — ponytail (github.com/DietrichGebert/ponytail, MIT) packaged as a
...
const PROVIDER_NAME = 'dsh-ponytail';
const SECTION_NAME = 'dsh-ponytail:ruleset';
...
async function buildCandidates() {
    if (!candidatesCache) {
        ...
        if (found.length === 0)
            throw new Error('[dsh-ponytail] no SKILL.md found under ' + SKILLS_ROOT + ' — bundled skills tree is broken');
...
export function apply(ctx, config = {}) {
    ...
    if (!LEVELS.includes(configMode) && configMode !== 'off')
        throw new Error('[dsh-ponytail] invalid config mode: ' + JSON.stringify(configMode) + ' — expected off|lite|full|ultra');
    ...
    try {
        ...
        throw new Error('[dsh-ponytail] ruleset unreadable at ' + UPSTREAM_RULESET + ': ' + String(error));
    }
    ...
    if (ctx.skills) {
        ctx.effect(() => ctx.skills.registerProvider(() => provider), 'dsh-ponytail: skill provider');
        buildCandidates()
            .then((list) => log.info?.('[dsh-ponytail] serving ' + list.length + ' skills: ' + list.map((c) => c.name).join(', ')))
            .catch((error) => log.warn?.('[dsh-ponytail] skill discovery failed: ' + String(error)));
    }
    else {
        log.warn?.('[dsh-ponytail] skills service missing — skill provider not registered');
    }
    ...
    if (ctx.systemPrompt) {
        ...
        }), 'dsh-ponytail: ruleset section');
    }
    else if (configMode !== 'off') {
        log.warn?.('[dsh-ponytail] systemPrompt service missing — always-on injection disabled (config mode=' + configMode + ')');
    }
    ...
    if (ctx.commands) {
        ...
        }), 'dsh-ponytail: /ponytail command');
    }
    else {
        log.warn?.('[dsh-ponytail] commands service missing — /ponytail not registered');
    }

    log.info?.('[dsh-ponytail] ready (config mode=' + configMode + ')');
}
